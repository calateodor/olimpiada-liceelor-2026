...
Lansarea (făcută pe 25 sept 2026): `VOTE_LIVE = true` în `app/src/lib/vote.ts`, voturile de test șterse, `npm run deploy:pages`, plus o republicare pe `--branch vot-test` ca adresa de test să nu mai primească voturi. După lansare se votează doar pe olimpiada.primariaslatina.ro (`VOTE_HOST`): pe adresele pages.dev votul nu apare, ca un telefon să nu aibă câte un vot pe fiecare adresă. Închiderea e automată la `config.vote.closesAt` (implicit 1 oct 2026, 23:59:59, ora României), cu anunțarea primelor două (`STAGE_SEATS`), care urcă pe scenă; la egalitate pe locul 2 apar toate cele la egalitate. Opțional, verificarea anti-robot Cloudflare Turnstile: cheile se pun în panou, la Vot hostess.
