  ...
  if (!token) {
    return (
      <div className="pn-login">
        <form onSubmit={async e => { e.preventDefault(); setErr(''); setBusy(true); const r = await login(user, pw); setBusy(false); if (!r.ok) setErr(r.error ?? 'Nu ai intrat.'); }} className="pn-login-box">
          ...
          <label className="pn-field"><span className="mono">Utilizator</span><input type="text" value={user} onChange={e => setUser(e.target.value)} autoFocus autoComplete="username" autoCapitalize="off" /></label>
          <label className="pn-field"><span className="mono">Parolă</span><input type="password" value={pw} onChange={e => setPw(e.target.value)} autoComplete="current-password" /></label>
  ...
  return (
    <div className="pn">
      <aside className="pn-side">
        <Link to="/" className="pn-brand"><img src={asset('/img/medalioane.png')} alt="" width="64" /><span><b>Administrare</b><span className="mono">Olimpiada 2026</span></span></Link>
        <nav className="pn-tabs">{TABS.map(([t, l, ic]) => <button key={t} className={`pn-tab ${tab === t ? 'is-on' : ''}`} onClick={() => setTab(t)}><Icon icon={ic} width="20" />{l}</button>)}</nav>
      ...
      <div className="pn-main">
        <header className="pn-bar">
          ...
          <div className="row">
            {msg && <span className="mono pn-msg">{msg}</span>}
...
function Acasa({ go }: { go: (t: Tab) => void }) {
  ...
  return (
    <div className="pn-sec">
      {!online && <div className="pn-note"><Icon icon="solar:info-circle-linear" /> Site-ul rulează static (GitHub Pages): panoul funcționează complet, dar modificările rămân în acest browser până când site-ul e mutat pe Cloudflare. Vezi <code>DEPLOY.md</code>.</div>}
      <div className="pn-stats">
        <div className="pn-stat"><b>{live.length}</b><span>meciuri live</span></div>
        <div className="pn-stat"><b>{todayMs.length}</b><span>meciuri azi</span></div>
        <div className="pn-stat"><b>{played}<i>/{state.matches.length}</i></b><span>meciuri jucate</span></div>
        <div className="pn-stat"><b>{evDone}<i>/{state.events.length}</i></b><span>probe încheiate</span></div>
        <div className="pn-stat"><b>{state.photos.length}</b><span>poze</span></div>
        <div className="pn-stat"><b>{state.timeline.length}</b><span>noutăți</span></div>
        <div className={`pn-stat ${overdue.length ? 'is-warn' : ''}`}><b>{overdue.length}</b><span>rezultate lipsă</span></div>
        <div className={`pn-stat ${dirty ? 'is-warn' : ''}`}><b>{dirty ? '!' : '✓'}</b><span>{dirty ? 'nepublicat' : 'totul publicat'}</span></div>
      </div>
        ...
        {todayMs.length === 0 ? <p className="body dim">Nu sunt meciuri programate azi.</p> : (
          <div className="pn-rows">
            {todayMs.map(rm => {
              ...
              return (
                <div key={m.id} className={`pn-quick is-${m.status}`}>
                  <span className="mono">{m.time}</span>
                  <span className="pn-quick-t"><b>{evName(state, m.eventId)}</b><span className="dim"> · {STAGE_LABEL[m.stage]} · {m.venue}</span></span>
                  <span className="pn-team">{rm.home ? <><SchoolMark school={SCHOOL_BY_ID[rm.home]} size="sm" plain />{SCHOOL_BY_ID[rm.home].short}</> : rm.homeLabel}</span>
                  <input className="pn-score" type="number" min="0" value={m.homeScore ?? ''} onChange={e => upd(m.id, { homeScore: e.target.value === '' ? null : Number(e.target.value) }, `Scor ${lbl(m)}`)} />
                  <span className="mono">:</span>
                  <input className="pn-score" type="number" min="0" value={m.awayScore ?? ''} onChange={e => upd(m.id, { awayScore: e.target.value === '' ? null : Number(e.target.value) }, `Scor ${lbl(m)}`)} />
                  <span className="pn-team">{rm.away ? <>{SCHOOL_BY_ID[rm.away].short}<SchoolMark school={SCHOOL_BY_ID[rm.away]} size="sm" plain /></> : rm.awayLabel}</span>
                  <span className="row">
      ...
      <div className="pn-two">
        <section className="pn-block">
          ...
          {overdue.length === 0 ? <p className="body dim">Toate meciurile trecute au rezultat.</p> : <ul className="pn-list pn-list-3">{overdue.map(m => <li key={m.id}><span className="mono">{fmtDate(m.date)} {m.time}</span><b>{lbl(m)}</b><button className="link" onClick={() => go('meciuri')}>completează</button></li>)}</ul>}
        </section>
        <section className="pn-block">
          <h3 className="h4">Urmează</h3>
          <ul className="pn-list pn-list-2">{upcoming.map(m => <li key={m.id}><span className="mono">{fmtDate(m.date)} {m.time}</span><b>{lbl(m)}</b></li>)}</ul>
        </section>
      </div>

      <section className="pn-block">
        <h3 className="h4">Scurtături</h3>
        <div className="row pn-shortcuts">
...
function Meciuri() {
  ...
  return (
    <div className="pn-sec">
      <div className="row pn-evsel">{evs.map(e => <button key={e.id} className={`tag ${e.id === evId ? 'tag-solid' : ''}`} onClick={() => { setEvId(e.id); setDay(''); }}>{e.name} {e.subtitle.split(' ')[0]}</button>)}</div>
      <div className="row">
      ...
      {stages.map(st => {
        ...
        return (
          <section key={st} className="pn-block">
            ...
            <div className="pn-rows">
              {ms.map(rm => {
                ...
                return (
                  <div key={m.id} className={`pn-match is-${m.status}`}>
                    <div className="pn-match-when"><input type="date" value={m.date} onChange={e => upd(m.id, { date: e.target.value }, `Dată ${lbl(m)}`)} /><input type="time" value={m.time} onChange={e => upd(m.id, { time: e.target.value }, `Oră ${lbl(m)}`)} /><input type="text" value={m.venue} onChange={e => upd(m.id, { venue: e.target.value })} placeholder="Locație" />{editable && <button className="link pn-del" onClick={() => del(m)}>șterge</button>}</div>
                    <div className="pn-match-teams">
                      {editable ? <SchoolSelect value={m.home ?? rm.home} onChange={v => upd(m.id, { home: v }, `Echipă ${lbl(m)}`)} /> : <span className="pn-team">{m.home && <SchoolMark school={SCHOOL_BY_ID[m.home]} size="sm" plain />}{m.home ? SCHOOL_BY_ID[m.home].short : rm.homeLabel}</span>}
                      <input className="pn-score" type="number" min="0" value={m.homeScore ?? ''} onChange={e => upd(m.id, { homeScore: e.target.value === '' ? null : Number(e.target.value) }, `Scor ${lbl(m)}`)} aria-label="Scor gazde" />
                      <span className="mono">:</span>
                      <input className="pn-score" type="number" min="0" value={m.awayScore ?? ''} onChange={e => upd(m.id, { awayScore: e.target.value === '' ? null : Number(e.target.value) }, `Scor ${lbl(m)}`)} aria-label="Scor oaspeți" />
                      {editable ? <SchoolSelect value={m.away ?? rm.away} onChange={v => upd(m.id, { away: v }, `Echipă ${lbl(m)}`)} /> : <span className="pn-team">{m.away && <SchoolMark school={SCHOOL_BY_ID[m.away]} size="sm" plain />}{m.away ? SCHOOL_BY_ID[m.away].short : rm.awayLabel}</span>}
                    </div>
                    <div className="pn-match-ctl">
                      ...
                      <input type="text" className="pn-note-in" placeholder="notă publică (opțional)" value={m.note?.startsWith('pen:') ? '' : m.note ?? ''} onChange={e => upd(m.id, { note: e.target.value })} />
  ...
  return (
    <div className="pn-sec">
      ...
      {state.events.map(ev => {
        ...
        return (
          <section key={ev.id} className="pn-block">
            <div className="between">
              ...
              <div className="row">
                <label className="pn-check"><input type="checkbox" checked={ev.finished} onChange={e => set(x => { x.finished = e.target.checked; }, `${ev.name} ${ev.subtitle}: ${e.target.checked ? 'încheiată' : 'redeschisă'}`)} /> Încheiată (punctează în general)</label>
            ...
            <div className="pn-places">
              {PLACE_LABEL.map((lbl, i) => (
                <label key={i} className="pn-field"><span className="mono">Locul {lbl}{ev.format !== 'ranking' && auto[i] && !pl[i] ? ` · auto: ${SCHOOL_BY_ID[auto[i]!].short}` : ''}</span>
            ...
            {ev.format === 'ranking' && (
              <details className="pn-details"><summary className="mono">Punctaje / timpi afișate (opțional)</summary>
                <div className="pn-places">{SCHOOLS.map(s => <label key={s.id} className="pn-field"><span className="mono">{s.nr} · {s.short}</span><input type="text" value={ev.scores?.[s.id] ?? ''} placeholder="ex. 87 pct / 18:42" onChange={e => set(x => { x.scores = { ...(x.scores ?? {}), [s.id]: e.target.value }; }, `${ev.name}: punctaj ${s.short}`)} /></label>)}</div>
              </details>
            )}
            {open === ev.id && (
              <div className="pn-places">
                <label className="pn-field"><span className="mono">Locul de desfășurare</span><input type="text" value={ev.venue} onChange={e => set(x => { x.venue = e.target.value; }, `${ev.name}: loc`)} /></label>
                <label className="pn-field"><span className="mono">Perioada (text afișat)</span><input type="text" value={ev.dateLabel} onChange={e => set(x => { x.dateLabel = e.target.value; }, `${ev.name}: perioadă`)} /></label>
                <label className="pn-field"><span className="mono">Începe</span><input type="date" value={ev.startDate} onChange={e => set(x => { x.startDate = e.target.value; }, `${ev.name}: start`)} /></label>
                <label className="pn-field"><span className="mono">Se termină</span><input type="date" value={ev.endDate} onChange={e => set(x => { x.endDate = e.target.value; }, `${ev.name}: final`)} /></label>
                <label className="pn-field"><span className="mono">Ora (probe de o zi)</span><input type="time" value={ev.time ?? ''} onChange={e => set(x => { x.time = e.target.value || undefined; }, `${ev.name}: oră`)} /></label>
                <label className="pn-field"><span className="mono">Echipa</span><input type="text" value={ev.teamSize ?? ''} onChange={e => set(x => { x.teamSize = e.target.value; })} /></label>
                <label className="pn-field pn-span"><span className="mono">Descriere (pagina probei)</span><textarea rows={3} value={ev.description} onChange={e => set(x => { x.description = e.target.value; }, `${ev.name}: descriere`)} /></label>
              </div>
            )}
            <label className="pn-field"><span className="mono">Notă publică (opțional)</span><input type="text" value={ev.notes ?? ''} onChange={e => set(x => { x.notes = e.target.value; })} /></label>
          </section>
        );
      })}
    </div>
  ...
  return (
    <div className="pn-sec">
      <section className="pn-block">
        ...
        <label className="pn-field"><span className="mono">Mesaj când e ascuns / notă sub clasament</span><input type="text" value={st.note} onChange={e => setState(s => { s.config.standings.note = e.target.value; })} placeholder="ex. Clasamentul se publică după validarea juriului." /></label>
      </section>
      <section className="pn-block">
        ...
        <table className="table pn-table">
          ...
          <tbody>{gen.map((r, i) => { const a = st.adjustments.filter(x => x.schoolId === r.school.id).reduce((n, x) => n + x.pts, 0); return <tr key={r.school.id}><td className="rank">{i + 1}</td><td><span className="pn-team"><SchoolMark school={r.school} size="sm" plain />{r.school.name}</span></td><td className="c num">{r.gold}</td><td className="c num">{r.silver}</td><td className="c num">{r.bronze}</td><td className="c num">{a ? (a > 0 ? `+${a}` : a) : '—'}</td><td className="pts">{r.pts}</td></tr>; })}</tbody>
        </table>
      </section>
      <section className="pn-block">
        ...
        <div className="pn-places">
          <label className="pn-field"><span className="mono">Liceu</span><SchoolSelect value={adj.schoolId} allowNull={false} onChange={v => setAdj({ ...adj, schoolId: v! })} /></label>
          <label className="pn-field"><span className="mono">Puncte (negativ = penalizare)</span><input type="number" value={adj.pts} onChange={e => setAdj({ ...adj, pts: Number(e.target.value) })} /></label>
          <label className="pn-field pn-span"><span className="mono">Motiv</span><input type="text" value={adj.reason} onChange={e => setAdj({ ...adj, reason: e.target.value })} /></label>
        </div>
      ...
      <section className="pn-block">
        ...
        <div className="pn-places">{PLACE_LABEL.map((l, i) => <label key={i} className="pn-field"><span className="mono">Locul {l}</span><input type="number" min="0" value={state.config.pointsPerPlace[i] ?? 0} onChange={e => setState(s => { s.config.pointsPerPlace[i] = Number(e.target.value); }, `Puncte locul ${l}: ${e.target.value}`)} /></label>)}</div>
      </section>
    </div>
  );
}
...
function Licee() {
  ...
  return (
    <div className="pn-sec">
      ...
      <section className="pn-block">
        <div className="pn-school-head"><SchoolCrest school={s} size="lg" badge /><div><h3 className="h4">{s.name}</h3><p className="mono">Nr. {s.nr} · {s.colorName} · Grupa {s.group} · locul {rank} în general</p></div></div>
        <div className="pn-places">
          <label className="pn-field"><span className="mono">Motto / slogan (pagina liceului)</span><input type="text" value={info.motto ?? ''} onChange={e => setInfo('motto', e.target.value)} /></label>
          <label className="pn-field"><span className="mono">Profesor coordonator</span><input type="text" value={info.coordinator ?? ''} onChange={e => setInfo('coordinator', e.target.value)} /></label>
          <label className="pn-field"><span className="mono">Contact (telefon / email)</span><input type="text" value={info.contact ?? ''} onChange={e => setInfo('contact', e.target.value)} /></label>
          <label className="pn-field pn-span"><span className="mono">Notă publică</span><input type="text" value={info.note ?? ''} onChange={e => setInfo('note', e.target.value)} placeholder="ex. Galeria liceului se adună la ora 16:30 la Baza Dobrescu." /></label>
        </div>
      </section>
      <section className="pn-block">
        ...
        <div className="pn-roster">
          {state.events.map(ev => (
            <label key={ev.id} className="pn-field"><span className="mono">{ev.name} {ev.subtitle}</span>
...
function Poze() {
  ...
  return (
    <div className="pn-sec">
      {!online && <p className="pn-err">Fără server: pozele se pot încărca doar când site-ul rulează pe Cloudflare (au nevoie de spațiu de stocare).</p>}
      <section className="pn-block">
        <h3 className="h4">Încarcă poze</h3>
        <div className="pn-places">
          <label className="pn-field"><span className="mono">Proba</span><select value={evId} onChange={e => setEvId(e.target.value as EventId)}><option value="">— fără —</option>{state.events.map(e => <option key={e.id} value={e.id}>{e.name} {e.subtitle}</option>)}</select></label>
          <label className="pn-field"><span className="mono">Liceu</span><select value={scId} onChange={e => setScId(e.target.value as SchoolId)}><option value="">— fără —</option>{SCHOOLS.map(s => <option key={s.id} value={s.id}>{s.nr} · {s.short}</option>)}</select></label>
          <label className="pn-field"><span className="mono">Descriere</span><input type="text" value={cap} onChange={e => setCap(e.target.value)} placeholder="opțional" /></label>
        </div>
        <label className="pn-drop"><input ref={fileRef} type="file" accept="image/*" multiple onChange={e => onFiles(e.target.files)} disabled={!online} /><Icon icon="solar:gallery-add-linear" width="32" /><span>Alege pozele (se redimensionează automat la 1800px)</span></label>
  ...
  return (
    <div className="pn-sec">
      <section className="pn-block">
        ...
        <div className="pn-places">
          <label className="pn-field"><span className="mono">Data</span><input type="datetime-local" value={t.date.slice(0, 16)} onChange={e => setT({ ...t, date: e.target.value })} /></label>
          <label className="pn-field"><span className="mono">Tip</span><select value={t.kind} onChange={e => setT({ ...t, kind: e.target.value as TimelineEntry['kind'] })}><option value="news">Noutate</option><option value="result">Rezultat</option><option value="milestone">Moment</option><option value="concert">Concert</option></select></label>
          <label className="pn-field"><span className="mono">Proba</span><select value={t.eventId ?? ''} onChange={e => setT({ ...t, eventId: (e.target.value || undefined) as EventId })}><option value="">—</option>{state.events.map(e => <option key={e.id} value={e.id}>{e.name} {e.subtitle}</option>)}</select></label>
          <label className="pn-field"><span className="mono">Liceu</span><select value={t.schoolId ?? ''} onChange={e => setT({ ...t, schoolId: (e.target.value || undefined) as SchoolId })}><option value="">—</option>{SCHOOLS.map(s => <option key={s.id} value={s.id}>{s.nr} · {s.short}</option>)}</select></label>
        </div>
        <label className="pn-field"><span className="mono">Titlu</span><input type="text" value={t.title} onChange={e => setT({ ...t, title: e.target.value })} /></label>
        <label className="pn-field"><span className="mono">Text</span><textarea rows={3} value={t.body} onChange={e => setT({ ...t, body: e.target.value })} /></label>
      ...
      <section className="pn-block">
        ...
        <ul className="pn-list">{[...state.timeline].sort((a, b) => b.date.localeCompare(a.date)).map(e => <li key={e.id}><span className="mono">{fmtDate(e.date, 'day')}</span><b>{e.title}</b><span className="dim">{e.body}</span><span className="row"><button className="link" onClick={() => { setT({ ...e, date: e.date.slice(0, 16) }); setEditing(e.id); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>modifică</button><button className="link pn-del" onClick={() => setState(s => { s.timeline = s.timeline.filter(x => x.id !== e.id); }, `Noutate ștearsă: ${e.title}`)}>șterge</button></span></li>)}</ul>
      </section>
    </div>
  );
}
  ...
  return (
    <div className="pn-sec">
      <section className="pn-block">
        ...
        <div className="pn-places">
          <label className="pn-field pn-span"><span className="mono">Text</span><input type="text" value={a.text} onChange={e => setState(s => { s.config.announcement.text = e.target.value; })} placeholder="ex. Finala de fotbal se mută la 18:30 din cauza ploii." /></label>
          <label className="pn-field"><span className="mono">Ton</span><select value={a.kind} onChange={e => setState(s => { s.config.announcement.kind = e.target.value as 'info'; }, `Anunț: ton ${e.target.value}`)}><option value="info">Informare (mov)</option><option value="live">Live / important (roșu)</option><option value="warn">Atenție (galben)</option></select></label>
          <label className="pn-field"><span className="mono">Link (opțional)</span><input type="text" value={a.link ?? ''} onChange={e => setState(s => { s.config.announcement.link = e.target.value; })} placeholder="/probe/fotbal sau https://…" /></label>
        </div>
      </section>
      <section className="pn-block">
      ...
      <section className="pn-block">
        ...
        <label className="pn-field"><span className="mono">Text</span><input type="text" value={m.text} onChange={e => setState(s => { s.config.maintenance.text = e.target.value; })} /></label>
      </section>
    </div>
  );
}
  ...
  return (
    <div className="pn-sec">
      ...
      <section className="pn-block">
        ...
        {c.concert.poster && <img src={c.concert.poster} alt="" className="pn-poster" />}
        <div className="row">
          ...
          {c.concert.poster && <button className="link pn-del" onClick={() => setState(s => { s.config.concert.poster = ''; }, 'Afiș concert scos')}>scoate afișul</button>}
        ...
        <label className="pn-field"><span className="mono">sau link direct la imagine</span><input type="text" value={c.concert.poster} onChange={e => setState(s => { s.config.concert.poster = e.target.value; })} placeholder="https://…/afis.jpg" /></label>
      </section>
    </div>
  );
}
  ...
  return (
    <div className="pn-sec">
      <section className="pn-block">
        ...
        <div className="pn-places">
          <label className="pn-field"><span className="mono">Titlul site-ului (tab, partajări)</span><input type="text" value={c.siteTitle} onChange={e => set('Titlu site', x => { x.siteTitle = e.target.value; })} /></label>
          <label className="pn-field"><span className="mono">Tagline sub „Hai la joc!”</span><input type="text" value={c.heroTagline} onChange={e => set('Tagline hero', x => { x.heroTagline = e.target.value; })} /></label>
          <label className="pn-field"><span className="mono">„Următorul eveniment” din hero</span><select value={c.countdownEventId} onChange={e => set('Următorul eveniment forțat', x => { x.countdownEventId = e.target.value as EventId; })}><option value="">automat (cel mai apropiat meci)</option>{state.events.filter(e => e.format !== 'ranking').map(e => <option key={e.id} value={e.id}>{e.name} {e.subtitle}</option>)}</select></label>
        </div>
      ...
      <section className="pn-block">
        ...
        <div className="pn-places">
          {([['email', 'Email'], ['phone', 'Telefon'], ['site', 'Site'], ['address', 'Adresă'], ['facebook', 'Facebook (link)'], ['instagram', 'Instagram (link)'], ['tiktok', 'TikTok (link)'], ['youtube', 'YouTube (link)']] as const).map(([k, l]) => (
            <label key={k} className="pn-field"><span className="mono">{l}</span><input type="text" value={c.contact[k]} onChange={e => set(`Contact: ${l}`, x => { x.contact[k] = e.target.value; })} /></label>
          ))}
        </div>
      </section>
    </div>
  ...
  return (
    <div className="pn-sec">
      <section className="pn-block">
        ...
        <div className="pn-places">{VEN.map(([id, name]) => <label key={id} className="pn-field"><span className="mono">{name}</span><input type="text" value={c.venueNotes[id] ?? ''} onChange={e => setState(s => { s.config.venueNotes[id] = e.target.value; }, `Notă locație: ${name}`)} placeholder="ex. intrarea prin str. Crișan, parcare la stadion" /></label>)}</div>
      </section>
      <section className="pn-block">
        <h3 className="h4">Regulamente vizibile</h3>
        <div className="pn-switches">{REGS.map(([slug, name]) => <Switch key={slug} on={!c.regsHidden.includes(slug)} onChange={v => setState(s => { s.config.regsHidden = v ? s.config.regsHidden.filter(x => x !== slug) : [...s.config.regsHidden, slug]; }, `Regulament ${name} ${v ? 'vizibil' : 'ascuns'}`)} label={name} />)}</div>
      </section>
      <section className="pn-block">
        ...
        <div className="pn-places">
          <label className="pn-field"><span className="mono">Titlu</span><input type="text" value={doc.title} onChange={e => setDoc({ ...doc, title: e.target.value })} /></label>
          <label className="pn-field"><span className="mono">Link</span><input type="text" value={doc.url} onChange={e => setDoc({ ...doc, url: e.target.value })} placeholder="https://…" /></label>
        </div>
        ...
        {c.extraDocs.length > 0 && <ul className="pn-list pn-list-3">{c.extraDocs.map(d => <li key={d.id}><b>{d.title}</b><a className="link dim" href={d.url} target="_blank" rel="noreferrer">{d.url}</a><button className="link pn-del" onClick={() => setState(s => { s.config.extraDocs = s.config.extraDocs.filter(x => x.id !== d.id); }, `Document șters: ${d.title}`)}>șterge</button></li>)}</ul>}
      </section>
    </div>
  );
}
  ...
  return (
    <div className="pn-sec">
      <section className="pn-block">
        ...
        {!online && <p className="pn-note"><Icon icon="solar:info-circle-linear" /> Fără server, parola nu se poate schimba de aici. Rămâne cea din configurare până la mutarea pe Cloudflare.</p>}
        <div className="pn-places">
          <label className="pn-field"><span className="mono">Utilizator</span><input type="text" value={u} onChange={e => setU(e.target.value)} autoComplete="username" /></label>
          <label className="pn-field"><span className="mono">Parolă nouă (min. 10)</span><input type="password" value={p1} onChange={e => setP1(e.target.value)} autoComplete="new-password" /></label>
          <label className="pn-field"><span className="mono">Repetă parola</span><input type="password" value={p2} onChange={e => setP2(e.target.value)} autoComplete="new-password" /></label>
        </div>
        <div className="row">
          ...
          {p1 && p2 && p1 !== p2 && <span className="pn-err">Parolele nu coincid.</span>}
  ...
  return (
    <div className="pn-sec">
      <section className="pn-block">
        ...
        {state.log.length === 0 ? <p className="body dim">Nicio modificare încă.</p> : <ul className="pn-log">{state.log.slice(0, 80).map((l, i) => <li key={i}><span className="mono">{new Date(l.at).toLocaleString('ro-RO', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</span><span>{l.what}</span></li>)}</ul>}
      </section>
      <div className="pn-two">
        <section className="pn-block"><h3 className="h4">Export</h3><p className="body">Toată baza de date (meciuri, rezultate, poze, setări) ca fișier JSON. Fă unul înainte de modificări mari.</p><button className="btn btn-sm" onClick={download}><Icon className="ic" icon="solar:download-minimalistic-linear" /> Descarcă JSON</button></section>
        <section className="pn-block"><h3 className="h4">Import</h3><p className="body">Încarcă un JSON exportat anterior. Înlocuiește tot; publică după.</p><input type="file" accept="application/json" onChange={e => importFile(e.target.files?.[0])} />{txt && <p className="mono">{txt}</p>}</section>
      </div>
      <section className="pn-block">
