...
export function TimeMachine() {
  ...
  return (
    <div className="ad-sec">
      <section className={`ad-block ${sim.on ? 'ad-tm-on' : ''}`}>
        <div className="between">
          ...
          <label className={`ad-switch ${sim.on ? 'is-on' : ''}`}>
            ...
            <span className="ad-switch-k" aria-hidden="true" /><span className="ad-switch-t"><b>{sim.on ? 'Simulare activă' : 'Simulare oprită'}</b></span>
          </label>
        </div>
        {sim.on && <p className="ad-note"><Icon icon="solar:clock-circle-linear" /> Acum, în simulare: <b>{fmtNow(cur)}</b>{sim.frozen ? ' · ceas înghețat' : ' · ceasul curge'}. Ce vezi pe site (și în celelalte taburi) sunt date fictive. Apasă „Publică” dacă vrei să vadă și alții; vizitatorii primesc o bandă „Simulare · date fictive”.</p>}
      </section>

      <section className="ad-block">
        <h3 className="h4">Momentul</h3>
        <div className="ad-tm-row">
          <label className="ad-field"><span className="mono">Ziua</span><input type="date" value={local.date} min="2026-09-07" max="2026-10-05" onChange={e => setAt(fromLocal(e.target.value, local.time))} /></label>
          <label className="ad-field"><span className="mono">Ora</span><input type="time" value={local.time} onChange={e => setAt(fromLocal(local.date, e.target.value))} /></label>
          <div className="ad-field"><span className="mono">Sari</span><span className="row">
          ...
          <div className="ad-field"><span className="mono">Ceasul</span><label className="ad-check"><input type="checkbox" checked={sim.frozen} onChange={e => set({ frozen: e.target.checked }, e.target.checked ? 'Ceas înghețat' : 'Ceasul curge')} /> înghețat la ora aleasă</label></div>
        </div>
        <p className="mono dim">Zilele competiției:</p>
        <div className="row ad-tm-days">
          ...
          {!sim.on ? <p className="body dim">Pornește simularea ca să vezi rezumatul momentului.</p> : (
            <>
              <div className="ad-stats ad-stats-3">
                <div className={`ad-stat ${live.length ? 'is-warn' : ''}`}><b>{live.length}</b><span>live</span></div>
                <div className="ad-stat"><b>{played}<i>/{state.matches.length}</i></b><span>meciuri jucate</span></div>
                <div className="ad-stat"><b>{doneEv}<i>/{state.events.length}</i></b><span>probe încheiate</span></div>
              </div>
              {live.length > 0 && <ul className="ad-list ad-list-2">{live.map(m => <li key={m.id}><span className="mono">{m.time}</span><b>{evName(m.eventId)} · {STAGE_LABEL[m.stage]}: {m.home ? SCHOOL_BY_ID[m.home].short : '?'} {m.homeScore}–{m.awayScore} {m.away ? SCHOOL_BY_ID[m.away].short : '?'}</b></li>)}</ul>}
